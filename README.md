...
- 在 Feishu 中通过文本、菜单和卡片按钮继续发起 Copilot 对话
- 接管本机已有会话并延续上下文

## 功能特性

- Telegram 机器人接入与 Chat ID 白名单控制
- Feishu 长连接机器人接入、菜单卡片交互与会话白名单控制
- Copilot 会话功能与多会话切换
...
Feishu 当前复用同一套 `TaskRunner`、会话持久化和 Copilot 调用链。收到文本或 `post` 富文本消息后会直接进入当前会话上下文；机器人自定义菜单、模型切换、会话列表、会话预览与历史查看使用卡片，普通对话过程与最终回复使用文本消息分段发送，以兼顾移动端可读性与飞书接口稳定性。
