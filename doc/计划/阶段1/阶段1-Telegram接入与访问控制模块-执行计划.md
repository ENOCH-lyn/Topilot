...
6. 模型菜单、会话菜单、会话详情、历史页和删除确认页均由独立渲染函数生成文本与按钮，便于自动化测试逐项断言。
...
6. `/model` 返回的模型按钮必须去除空值和重复值，当前模型按钮必须带 `✓` 标识。
7. 会话菜单分页必须能处理负数、过大页码和非法页码，最终展示页码必须落在合法范围内。
8. 会话详情页必须包含接管、查看历史、刷新详情、删除、返回列表按钮；历史页必须包含刷新历史、返回详情、返回列表按钮；删除页必须包含确认删除、取消、返回列表按钮。
...
5. 对模型菜单、会话菜单、详情页、历史页、删除确认页的文本和按钮结构做静态断言。
6. 对回调 payload 和页码解析做边界断言。
