...
## 1. 模块定位
该模块是当前课程版本的平台接入主入口，负责 Telegram Application 构建、命令注册、消息接收和白名单校验；同时与 Feishu 长连接接入共同组成双平台消息入口。其设计目标是在保证个人使用安全边界的前提下，提供清晰、稳定、可移动端使用的 Bot 交互体验。
...
6. 与 Feishu 接入共用 `TaskRunner`、会话存储和 Copilot 调用链。
...
7. 回调 payload 和分页参数统一解析，非法页码回退到第 0 页，避免按钮异常数据导致 handler 崩溃。
...
4. 带代理和不带代理两组配置。
5. 命令消息与普通文本消息样例。
6. 重复模型列表、空模型值、非法回调页码、多页会话列表和 `nav:` 导航回调样例。
...
6. 对 `nav:`、`model_sel:`、`smenu:` 等回调 payload 和页码解析做边界断言。
