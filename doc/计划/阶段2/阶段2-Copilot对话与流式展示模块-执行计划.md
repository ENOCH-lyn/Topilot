...
7. 当 Copilot 通过 `ask_user` 请求补充信息或权限确认时，挂起当前轮次并等待 Telegram 用户继续作答。
...
6. `ask_user` 事件会被解析为待确认问题，保存问题文本、候选选项和所属会话 ID，并在 Telegram 中生成提示文本或确认按钮。
...
8. 当出现 `ask_user` 事件时，系统必须保存待确认问题；用户下一条文本或候选按钮点击必须在原 `session_id` 中继续执行，而不是新建会话。
...
6. 含 `ask_user` 问题文本、候选选项和权限确认语义的 JSONL 事件样例。
...
6. 通过伪造 `ask_user` 事件与待确认回复，验证挂起、恢复与会话续跑逻辑。
