...
2. `tests/test_stores.py`：覆盖对话历史裁剪、会话激活、会话前缀唯一性、模型保存、会话删除、状态摘要、当前会话摘要、实时本机会话元数据优先级、本机会话前缀接管和提交流程中的默认会话元数据落地。
...
5. `tests/test_telegram_helpers.py`：覆盖模型按钮布局、文本分块、会话菜单渲染、白名单放行和未授权拒绝。
...
4. 2026-05-17 的本地基线结果为 `pytest` 执行 29 项全部通过，可作为当前仓库版本的基础测试记录。
...
## 9. 后续补强建议
1. 为 `telegram_bot.py` 的真实 handler 注册表和回调分支补齐更细粒度测试。
